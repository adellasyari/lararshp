                        ...
                        <div class="card-body">
                            ...
                            <div class="mb-3">
                                ...
                                <select name="idpet" id="idpet" class="form-select @error('idpet') is-invalid @enderror" required autofocus>
                                    ...
                                    @foreach($pets as $pet)
                                        <option value="{{ $pet->idpet }}" {{ old('idpet') == $pet->idpet ? 'selected' : '' }}>{{ $pet->nama }} - {{ $pet->pemilik->user->nama ?? 'Pemilik N/A' }}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select name="idrole_user" id="idrole_user" class="form-select @error('idrole_user') is-invalid @enderror" required>
                                    ...
                                    @foreach($dokters as $dokter)
                                        <option value="{{ $dokter->idrole_user }}" {{ old('idrole_user') == $dokter->idrole_user ? 'selected' : '' }}>{{ $dokter->user->nama ?? $dokter->user->name ?? 'Dokter' }}</option>
                                    @endforeach
                                </select>
